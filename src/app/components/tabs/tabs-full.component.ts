...
export class TabsFullComponent implements AfterContentInit, OnDestroy {
  ...
  public closeTab(tab: TabComponent, fromTabButton: boolean = false): void {
    ...
    // 1️⃣ - Se `hasCloseableEmit` for `true`, emite o evento ao invés de fechar diretamente
    ...
    const tabs = this.tabs?.toArray();
    const index = tabs.findIndex(t => t.code === tab.code);

    if (index === -1) {
      console.warn(`Aba "${tab.tabTitle}" não encontrada para remoção.`);
      return;
    }

    console.log(`Fechando aba "${tab.tabTitle}" no índice ${index}`);

    // 2️⃣ - Se esta aba estiver ativa, tenta ativar outra antes de removê-la
    if (tab.active) {
      const newIndex = index > 0 ? index - 1 : (tabs.length > 1 ? index + 1 : -1);
      if (newIndex !== -1) {
        this.selectTab(tabs[newIndex]);
      }
    }

    // 3️⃣ - Se for uma aba dinâmica, remove corretamente
    if (tab.isDynamicTab) {
      this.removeDynamicTabByIndex(tab.dynamicIndex);
    }

    // 4️⃣ - Remove a aba da lista
    tabs.splice(index, 1);
    this.tabs?.reset(tabs);

    // 5️⃣ - Se não há mais abas, limpa o contêiner dinâmico
    if (tabs.length === 0) {
  ...
  private removeDynamicTabByIndex(index: number): void {
    if (index < 0 || index >= this.cloneDynamicInstance.length) {
      console.warn(`Índice inválido para remoção: ${index}`);
      return;
    }

    console.log(`Removendo aba dinâmica no índice ${index}`);

    this.cloneDynamicInstance.splice(index, 1);

    if (this.dynamicTabPlaceholder?.viewContainer && index < this.dynamicTabPlaceholder.viewContainer.length) {
      try {
        this.dynamicTabPlaceholder.viewContainer.remove(index);
      } catch (error) {
        console.error(`Erro ao remover aba dinâmica no índice ${index}:`, error);
      }
    }
  }
